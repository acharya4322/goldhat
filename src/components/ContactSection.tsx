...
const ContactSection: React.FC<ContactSectionProps> = ({ darkMode = true }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const emailBody = `
New Contact Form Submission from Gold Hat Agency Website:
...
Submitted at: ${new Date().toLocaleString()}
      ...
      // Create mailto link
      const mailtoLink = `mailto:goldhatindia@gmail.com?subject=New Contact Form Submission&body=${encodeURIComponent(emailBody)}`;
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16">
          ...
          <div>
            ...
            <div className="bg-gradient-to-r from-yellow-500/20 to-yellow-600/10 p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-yellow-500/30">
              ...
              <button 
                onClick={() => window.open('mailto:goldhatindia@gmail.com?subject=Free Strategy Call Request&body=Hi, I would like to schedule a free 30-minute strategy call to discuss my marketing goals.', '_blank')}
                className="bg-gradient-to-r from-yellow-500 to-yellow-600 text-black px-4 sm:px-6 py-2 sm:py-3 rounded-full font-bold hover:shadow-lg transition-all duration-300 hover:scale-105 text-sm sm:text-base"
              >
                Schedule Now
              </button>
            </div>
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              {submitStatus === 'error' && (
                <div className="flex items-center space-x-2 bg-red-500/20 border border-red-500/30 rounded-xl p-3 sm:p-4">
                  ...
                  <span className="text-red-400 text-sm sm:text-base">Please fill in all required fields or try emailing us directly.</span>
                </div>
              )}
